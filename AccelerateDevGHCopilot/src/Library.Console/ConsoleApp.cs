...
public class ConsoleApp
{
    ...
    async Task<ConsoleState> SearchBooks()
    {
        ...
        Book? book = await _bookRepository.GetBookByTitle(bookTitle);
        ...
        List<BookItem> bookItems = await _bookRepository.GetBookItemsByBookId(book.Id);
        ...
        // Check if any book item is currently on loan (not returned)
        ...
        foreach (BookItem bookItem in bookItems)
        {
            ...
            {
                if (loan.BookItemId == bookItem.Id && loan.ReturnDate == null)
                {
                    isAvailable = false;
                    activeLoan = loan;
                    break;
